...
fs.readdir(directoryPath, function (err, files) {
  ...
  console.log(
    "Etape 0 : Listage des fichiers .png dans '" + directoryPath + "'"
  );
        ...
        list.forEach((element) => {
          ...
          if (
            ...
            fs.writeJSONSync(
              path.join(
                folderBase,
                file.substring(0, file.length - 3) + "json"
              ),
              json,
              { spaces: 4 }
            );
            ...
            ("tEXt" == element.type) &
            element.data.startsWith("invokeai_metadata")
          ) {
            const text = element.data.substring("invokeai_metadata".length + 1);
            let json = JSON.parse(text);
            for (const key in json.image) {
              if (Object.hasOwnProperty.call(json.image, key)) {
                const attribut = json.image[key];
