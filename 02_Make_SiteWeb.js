        ...
        if (Object.hasOwnProperty.call(element.data, key2)) {
          ...
          try {
            if(typeof(element2)=="object"){
              LesItems=LesItems+Items.replaceAll("%%Titre%%",key2).replaceAll("%%Texte%%",JSON.stringify(element2,undefined,3))
            } else if (typeof(element2)=="array"){
              let strText =""
              element2.forEach((subelement2) => {
                if(typeof(subelement2)=="object" || typeof(subelement2)=="array" ){
                  strText+=JSON.stringify(subelement2,undefined,3)+","
                } else{
                  strText+=subelement2.toString()+","
                }
              })
              strText=strText.substring(0,strText.length-1)
              LesItems=LesItems+Items.replaceAll("%%Titre%%",key2).replaceAll("%%Texte%%",strText)
            }else {
